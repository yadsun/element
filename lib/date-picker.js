  ...
  methods: {
    ...
    mountPicker: function mountPicker() {
      ...
      var updateOptions = function updateOptions() {
        ...
        _this3.picker.disabledDates = options.disabledDates;
    ...
    rows: function rows() {
      ...
      for (var i = 0; i < 6; i++) {
        ...
        var _loop = function _loop(j) {
          ...
          var cellDisabledClass = ''; // 样式类名morning afternoon wholeday
          if (disabledDates && disabledDates.length) {
            for (var _i = 0; _i < disabledDates.length; _i++) {
              var obj = disabledDates[_i];
              var dateStr = obj.date;
              var usedArr = obj.used;
              var _date = new Date(dateStr);
              if (_date.getTime() === time) {
    ...
    markRange: function markRange(minDate, maxDate) {
      ...
      for (var i = 0, k = rows.length; i < k; i++) {
        ...
        for (var j = 0, l = row.length; j < l; j++) {
          ...
          var _time = Object(date_util_["nextDate"])(startDate, index - this.offsetDay).getTime();
